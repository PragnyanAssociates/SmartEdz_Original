...
app.get('/api/admin/study-materials/:instId/teacher/:userId', async (req, res) => {
    ...
    try {
        ...
        let query = `SELECT sm.*, cg.className, cg.section 
                     ...
                     LEFT JOIN classes cg ON sm.class_id = cg.id
                     WHERE sm.institutionId = ?`;
        let params = [instId];
...
app.post('/api/admin/study-materials', studyMaterialUpload.single('materialFile'), async (req, res) => {
    ...
    if (!institutionId || !title || !material_type || !uploaded_by) {
...
app.put('/api/admin/study-materials/:materialId', studyMaterialUpload.single('materialFile'), async (req, res) => {
    ...
    const { title, description, class_id, subject, material_type, external_link, existing_file_path } = req.body;
    ...
    try {
        const query = `UPDATE study_materials 
                       SET title=?, description=?, class_id=?, subject=?, material_type=?, file_path=?, external_link=? 
                       WHERE id=?`;
...
app.delete('/api/admin/study-materials/:materialId', async (req, res) => {
    try {
        const [[material]] = await db.execute(
            'SELECT file_path FROM study_materials WHERE id = ?', [req.params.materialId]
        );
