...
export default function PreAdmissionsScreen() {
  ...
  // Admin Check
  const isAdmin = user?.role === 'Super Admin' || user?.role === 'Developer';
            ...
            <div className="bg-white rounded-3xl border border-slate-100 shadow-sm h-[80vh] flex flex-col">
              <div className="p-6 border-b border-slate-50 flex items-center gap-4 bg-slate-50/50 rounded-t-3xl shrink-0">
                ...
                <div className="flex gap-2">
                  <button onClick={() => handleOpenModal(selectedItem)} className="p-2.5 bg-amber-50 text-amber-600 hover:bg-amber-100 rounded-xl transition-colors"><Edit size={16} /></button>
                  <button onClick={() => handleDelete(selectedItem.id)} className="p-2.5 bg-red-50 text-red-600 hover:bg-red-100 rounded-xl transition-colors"><Trash2 size={16} /></button>
                </div>
              </div>
